...
export default class jobsService {
  ...
  private createJobItem(item:any,jobPage:string):IJobsListItem{
    ...
    const res = {
      ...
      PublishDate:item.eldPublishDate ? new Date(item.eldPublishDate) : undefined,
      Url:{
  ...
  public async getAllJobs(context: ISPFXContext, listUrl: string, jobPage: string): Promise<IJobsListItem[]> {
    const sp = spfi().using(SPFx(context));
    let result:IJobsListItem[] = [];
    let now = new Date().toISOString();
    const items: any[] = await sp.web.getList(listUrl).items
                                     ...
                                     .expand("eldHiringManagerUser")
                                     .filter(`(eldPublishDate le datetime'${now}' or eldPublishDate eq null) and eldJobStatus eq 'בתוקף'`)()
    // Sort by eldPublishDate descending, null values at the end
    items.sort((a, b) => {
      if (!a.eldPublishDate) return 1;
      if (!b.eldPublishDate) return -1;
      return new Date(b.eldPublishDate).getTime() - new Date(a.eldPublishDate).getTime();
    });
